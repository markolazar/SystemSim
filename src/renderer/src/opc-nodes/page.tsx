...
export default function OPCNodesPage() {
  ...
  const [discoveryLog, setDiscoveryLog] = useState<
    Record<string, { last_discovered: number; duration_ms: number }>
  >({})
      ...
      try {
        ...
        if (data.success && data.children) {
          ...
          try {
            ...
            if (
              savedData.success &&
              savedData.selected_nodes &&
              savedData.selected_nodes.length > 0
            ) {
  ...
  const filteredChildren = useMemo(() => {
    return children.filter(
      (child) =>
        ...
        (child.browse_name &&
          child.browse_name.toLowerCase().includes(childrenSearch.toLowerCase()))
    )
  ...
  const toggleChildrenSort = (
    key: 'node_id' | 'browse_name' | 'last_discovered' | 'duration_ms'
  ) => {
  ...
  // The handleDiscoverNodes function has been removed as it was unused.
  ...
  return (
    <SidebarProvider>
                ...
                <div className="space-y-2">
                  ...
                  {isChildrenLoading ? (
                    ...
                    <p className="text-sm text-muted-foreground">
                      No child nodes found under this prefix.
                    </p>
                  ) : (
                    <div className="space-y-4">
                      {/* Search Box */}
                      ...
                      <div className="flex items-center justify-between gap-3 text-sm">
                        ...
                        <span className="text-muted-foreground">
                          Showing {(currentChildrenPage - 1) * childrenPageSize + 1}–
                          {Math.min(currentChildrenPage * childrenPageSize, sortedChildren.length)}{' '}
                          of {sortedChildren.length}
                        </span>
                      </div>
                        ...
                        <Table className="min-w-full">
                          <TableHeader>
                            <TableRow className="bg-muted">
                              ...
                              >
                                Last Discovered{' '}
                                {childrenSortKey === 'last_discovered' &&
                                  (childrenSortDir === 'asc' ? '↑' : '↓')}
                              </TableHead>
                              <TableHead
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        onClick={async () => {
                                          ...
                                          try {
                                            const res = await fetch(
                                              `http://localhost:${backendPort}/opc/discover-under`,
                                              {
                                                method: 'POST',
                                                headers: { 'Content-Type': 'application/json' },
                                                body: JSON.stringify({
                                            ...
                                            const nodesResponse = await fetch(
                                              `http://localhost:${backendPort}/opc/nodes`
                                            )
                                            ...
                                            try {
                                              const logRes = await fetch(
                                                `http://localhost:${backendPort}/opc/discovery-log`
                                              )
                                              const logData = await logRes.json()
                                              if (logData.success && logData.log)
                                                setDiscoveryLog(logData.log)
                                            } catch {
                                              /* empty */
                                            }
                                          } catch (e) {
                                      ...
                                      <Button
                                        ...
                                        onClick={async () => {
                                          try {
                                            const res = await fetch(
                                              `http://localhost:${backendPort}/opc/delete-under?parent_id=${encodeURIComponent(child.node_id)}`,
                                              { method: 'POST' }
                                            )
                                            const data = await res.json()
                                            setDiscoveryResult(data)
                    ...
                    <Table className="min-w-full">
                      <TableHeader>
                        <TableRow className="bg-muted">
                          ...
                          >
                            Short Node ID{' '}
                            {sortKey === 'short_node_id' && (sortDir === 'asc' ? '↑' : '↓')}
                          </TableHead>
                          <TableHead
                            className="cursor-pointer hover:bg-muted/80"
                          ...
                          >
                            Browse Name{' '}
                            {sortKey === 'browse_name' && (sortDir === 'asc' ? '↑' : '↓')}
                          </TableHead>
                          <TableHead
                            className="cursor-pointer hover:bg-muted/80"
                  ...
                  {!isNodesLoading && (
                    <div className="flex items-center justify-between mt-4 text-sm text-muted-foreground">
                      <span>
                        ...
                        {Math.min(currentPage * pageSize, sortedNodes.length)} of{' '}
                        {sortedNodes.length}
                      </span>
                      <div className="flex items-center gap-2">
                        <button
      ...
      <Dialog open={discoveringNodeId !== null} onOpenChange={() => { }}>
        <DialogContent
          className="sm:max-w-md [&_button]:hidden"
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center gap-4 py-6">
            ...
            <div className="text-center">
              <p className="text-sm text-muted-foreground">Discovering child nodes for:</p>
